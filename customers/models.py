...
class AudiogramSession(models.Model):
    """오디오그램(청력도) 검사 1회 기록.

    - 같은 날짜에 여러 번 검사 가능
    - 기본 노출은 created_at 최신 1건
    """
    ...
    customer = models.ForeignKey(
        ...
        related_name="audiogram_sessions",
        verbose_name="고객",
    )
    test_datetime = models.DateTimeField("검사일시", default=timezone.now)
    points_mode = models.IntegerField("포인트", choices=POINTS_MODE_CHOICES, default=7)

    created_at = models.DateTimeField("생성일시", default=timezone.now)
    ...
    def __str__(self) -> str:
        return f"AudiogramSession({self.customer_id}, {self.created_at})"


class AudiogramPoint(models.Model):
    """주파수별 좌/우 dB 값."""
    ...
    frequency_hz = models.IntegerField("주파수(Hz)")
    # 기도
    right_db = models.IntegerField("기도(우)", null=True, blank=True)
    left_db = models.IntegerField("기도(좌)", null=True, blank=True)

    # 차폐
    mask_right_db = models.IntegerField("차폐(우)", null=True, blank=True)
    mask_left_db = models.IntegerField("차폐(좌)", null=True, blank=True)

    # 골도
    bone_right_db = models.IntegerField("골도(우)", null=True, blank=True)
    bone_left_db = models.IntegerField("골도(좌)", null=True, blank=True)
    ...
    def __str__(self) -> str:
        return f"AudiogramPoint({self.session_id}, {self.frequency_hz})"
