...
def dashboard(request: HttpRequest) -> HttpResponse:
    ...
    # bucket = {k: 0 for k in keys}  <-- 기존 단일 버킷 제거
    bucket_blue = {k: 0 for k in keys}   # 제품/결제 + 유상 A/S + 공단 인정금액
    bucket_yellow = {k: 0 for k in keys} # 후기적합
    ...
    def _bucket_key_for_date(d: datetime.date) -> datetime.date | None:
        if period == "month":
            ...
            return k if k in bucket_blue else None
        return d if d in bucket_blue else None

    # 1. Product Sales (Blue) + Follow-up (Yellow)
    # [수정] 제품/결제 탭 매출: '수납(PaymentTransaction)' 기준 (paid_at)
    # 기존 'cases' 루프에서 purchase_date 기준으로 잡던 로직 제거 -> PaymentTransaction 쿼리로 변경
    
    # (1) Blue: 제품/결제 + 유상 A/S + 공단 인정금액
    # PaymentTransaction: 제품/결제 탭에서 발생한 실제 결제/환불 이력 (paid_at 기준)
    pay_qs = PaymentTransaction.objects.filter(
        case__customer__is_deleted=False,
        paid_at__gte=date_min,
        paid_at__lte=date_max,
    )
    for p in pay_qs:
        k = _bucket_key_for_date(p.paid_at)
        if k is None:
            continue
    ...
    # AfterService: 유상 A/S 수납 이력 (paid_at 기준, 환불 차감 순액 반영)
    as_qs = AfterService.objects.filter(
        customer__is_deleted=False,
        is_paid=True,
        payment_status="PAID",
        paid_at__gte=date_min,
    ...
    for a in as_qs:
        k = _bucket_key_for_date(a.paid_at)
        if k is None:
            continue
        gross = int(a.amount or 0)
        refund = int(getattr(a, "refund_amount", 0) or 0)
        net = gross - refund
        if metric == "sales":
            bucket_blue[k] += net
        else:
    ...
    # 공단 인정금액: 케이스 구매일(없으면 추가 구매일) 기준으로 블루에 포함
    for c in cases:
        nhis_amt = int(getattr(c, "nhis_amount", 0) or 0)
        base_date = getattr(c, "purchase_date", None) or getattr(c, "purchase_date_add", None)
        if nhis_amt <= 0 or not base_date:
            continue
        if not (date_min <= base_date <= date_max):
            continue
        k = _bucket_key_for_date(base_date)
        if k is None:
            continue
        if metric == "sales":
            bucket_blue[k] += nhis_amt
        else:
            bucket_blue[k] += 1

    # (2) Yellow: 후기적합 (입금일 기준) - 기존 로직 유지 (CustomerCase 루프 필요)
    # 케이스 루프는 후기적합 집계를 위해 다시 돕니다.
    for c in cases:
        for n in [1, 2, 3, 4]:
            fu_date = getattr(c, f"fu{n}_deposit_date", None)
            fu_amt = getattr(c, f"fu{n}_deposit_amount", 0)
            
            if fu_date and date_min <= fu_date <= date_max:
                k = _bucket_key_for_date(fu_date)
                if k is not None:
                    if metric == "sales":
                        bucket_yellow[k] += int(fu_amt or 0)
                    else:
                        bucket_yellow[k] += 1


    # 2. A/S Sales (Blue)
    # A/S는 위에서 이미 처리했으므로 중복 집계 방지를 위해 이 섹션은 제거합니다.
    # (주석: as_qs는 위 1. Blue 집계 블록으로 병합됨)


    # Merge for Total (Line Chart uses Total)
    values = []
    for k in keys:
        values.append(bucket_blue[k] + bucket_yellow[k])
    ...
    # ... (Tooltip logic update needed?) ...
    # Tooltip logic remains based on 'values' (Total) for the line chart points.
    ...
    # Dataset Construction for Stacked Bars
    chart_bars: list[dict] = []  # day 뷰 전용 바
    chart_dataset: list[dict] = []
    
    if period == "day":
        val_total = values[0] if values else 0
        vmax = max(val_total, 1)
        x0, x1 = 10, 710
        y_top, y_bot = 40, 180
        # 일(네모) 막대 너비를 절반 수준으로 축소
        bar_w = 120
        x = (x0 + x1) / 2 - bar_w / 2
        h = 0 if val_total == 0 else ((val_total / vmax) * (y_bot - y_top))
        y = y_bot - h
        lbl = labels[0] if labels else ""
        
        # day 뷰에서도 스택 처리를 위해 blue/yellow 값 계산
        # (values[0]은 이미 합계임. bucket에서 가져와야 함)
        k = keys[0]
        v_blue = bucket_blue.get(k, 0)
        v_yellow = bucket_yellow.get(k, 0)
        
        pct_blue = 0
        pct_yellow = 0
        if val_total > 0:
            pct_blue = (v_blue / val_total) * 100
            pct_yellow = (v_yellow / val_total) * 100
        ...
        chart_bars = [{
            ...
            "value_blue": v_blue,
            "value_yellow": v_yellow,
            "tip": f"{lbl} · {_fmt_value(val_total)}".strip(" ·"),
        }]

    for i, label in enumerate(labels):
        k = keys[i]
        val_blue = bucket_blue[k]
        val_yellow = bucket_yellow[k]
        val_total = val_blue + val_yellow
        
        # Heights (Stacked)
        # Total Height % based on max_value
        total_ratio = 0 if max_value <= 0 else (val_total / max_value)
        # Min height for visibility if > 0
        total_height_pct = 0
        if val_total > 0:
            total_height_pct = max(18, round(total_ratio * 100))
        
        # Split inner heights
        # If total is small (forced to 18%), we distribute proportionally
        pct_blue = 0
        pct_yellow = 0
        
        if val_total > 0:
            pct_blue = (val_blue / val_total) * 100
            pct_yellow = (val_yellow / val_total) * 100
            
        chart_dataset.append({
            "label": label,
            "value": val_total,
            "value_blue": val_blue,
            "value_yellow": val_yellow,
            "display": _fmt_value(val_total),
            "height_pct": total_height_pct,
            "pct_blue": pct_blue,
            "pct_yellow": pct_yellow,
            "is_zero": val_total == 0,
        })
    ...
    new_customers_total = Customer.objects.filter(
        is_deleted=False,
        created_at__date__gte=date_min,
        created_at__date__lte=date_max,
    ).count()
...
# --- 장기 정체(기존 규칙 유지)
    ...
    ctx = {
        ...
        "as_action_needed_count": as_followup_count,
        "as_rows": as_rows,
...
def customer_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    # --- 오디오그램(상담/검사 공통)
    # 사용자 요구사항(검사 탭): 7분법 고정 주파수
    FREQS_7 = [500, 1000, 1500, 2000, 3000, 4000, 6000]
    FREQS_11 = [125, 250, 500, 750, 1000, 1500, 2000, 3000, 4000, 6000, 8000]

    audiogram_sessions = list(
        AudiogramSession.objects.filter(customer=customer).order_by("-created_at", "-id")
    )

    selected_audiogram = None
    selected_audiogram_id = request.GET.get("ag")
    if selected_audiogram_id and str(selected_audiogram_id).isdigit():
        selected_audiogram = next((s for s in audiogram_sessions if s.id == int(selected_audiogram_id)), None)
    ag_force_edit = (request.GET.get("ag_edit") == "1")
    ag_is_new = (request.GET.get("ag_new") == "1")
    if selected_audiogram is None and audiogram_sessions:
        selected_audiogram = audiogram_sessions[0]

    audiogram_freqs = []
    audiogram_points_map: dict[int, dict] = {}
    audiogram_rows: list[dict] = []
    audiogram_rows_all11: list[dict] = []
    # 기본 표시는 11분법
    audiogram_mode = 11
    if selected_audiogram is not None:
        audiogram_mode = int(getattr(selected_audiogram, "points_mode", 7) or 7)
        audiogram_freqs = FREQS_11 if audiogram_mode == 11 else FREQS_7
        pts = list(AudiogramPoint.objects.filter(session=selected_audiogram).order_by("frequency_hz"))
        for p in pts:
            audiogram_points_map[int(p.frequency_hz)] = {
                "right_db": p.right_db,
                "left_db": p.left_db,
                "mask_right_db": getattr(p, "mask_right_db", None),
                "mask_left_db": getattr(p, "mask_left_db", None),
                "bone_right_db": getattr(p, "bone_right_db", None),
                "bone_left_db": getattr(p, "bone_left_db", None),
            }

        # 현재 모드(7/11)에 맞는 표시용 rows
        for f in audiogram_freqs:
            row = audiogram_points_map.get(int(f), {})
            audiogram_rows.append(
                {
                    "freq": int(f),
                    "right_db": row.get("right_db"),
                    "left_db": row.get("left_db"),
                    "mask_right_db": row.get("mask_right_db"),
                    "mask_left_db": row.get("mask_left_db"),
                    "bone_right_db": row.get("bone_right_db"),
                    "bone_left_db": row.get("bone_left_db"),
                }
            )

        # UI에서 7↔11 전환 시 즉시 표기할 수 있도록 11pt 전체 rows를 항상 제공합니다.
        for f in FREQS_11:
            row = audiogram_points_map.get(int(f), {})
            audiogram_rows_all11.append(
                {
                    "freq": int(f),
                    "right_db": row.get("right_db"),
                    "left_db": row.get("left_db"),
    ...
    # 검사 탭: 새 검사 생성 후 미입력 상태(모든 포인트 None)면 입력 UI 숨김 처리용 플래그
    exam_ag_has_any = False
    try:
        exam_ag_has_any = any(
            (
                (r.get("right_db") is not None)
                or (r.get("left_db") is not None)
                or (r.get("mask_right_db") is not None)
                or (r.get("mask_left_db") is not None)
                or (r.get("bone_right_db") is not None)
                or (r.get("bone_left_db") is not None)
            )
            for r in audiogram_rows_all11
        ) if audiogram_rows_all11 else False
    except Exception:
        exam_ag_has_any = False
    ...
    def _parse_db(v: str | None) -> tuple[bool, int | None]:
        """오디오그램 입력값 파싱.
    ...
    if request.method == "POST":
        ...
        if action == "save_customer_info":
            ...
            if form.is_valid():
                ...
                obj.save()
                update_customer_stage(customer)
                # ✅ 청력도는 '장애도'와 별도 저장(요구사항)
                # - 여기(save_exam)에서는 청력도를 절대 저장/갱신하지 않습니다.

                # 입력 오류가 없을 때만 redirect
                if required_popup is None:
                    extra = {}
                    if request.POST.get("ag_id"):
                        extra["ag"] = request.POST.get("ag_id")
                        extra["scroll"] = "exam-audiogram"
                    return redirect(_redirect_url("검사", case_obj=None, extra=extra))

                # 오류 팝업 노출을 위해 리렌더
                exam_form = CustomerExamForm(instance=customer)
            else:
                # 청력도 저장은 장애구분(장애도) 입력 여부와 무관하게 허용
                # (장애도 누락 팝업은 띄우지 않음)
                exam_form = form

        elif action == "save_audiogram":
            if tab != "검사":
                return HttpResponseForbidden("검사 탭에서만 가능합니다.")

            ag_id = request.POST.get("ag_id")
            if not (ag_id and str(ag_id).isdigit()):
                return redirect(_redirect_url("검사", extra={"scroll": "exam-audiogram"}))

            sess = AudiogramSession.objects.filter(id=int(ag_id), customer=customer).first()
            if sess is None:
                return redirect(_redirect_url("검사", extra={"scroll": "exam-audiogram"}))

            # 검사일시/포인트 모드
            dt_raw = (request.POST.get("ag_test_datetime") or "").strip()
            if dt_raw:
                try:
                    sess.test_datetime = datetime.datetime.fromisoformat(dt_raw)
                except Exception:
                    pass

            mode_raw = (request.POST.get("ag_points_mode") or "11").strip()
            if mode_raw not in ("7", "11"):
                mode_raw = "11"
            sess.points_mode = int(mode_raw)
            sess.save(update_fields=["test_datetime", "points_mode", "updated_at"])

            # 항상 11pt 전체를 보유(7↔11 전환/히스토리 안정성) — 없는 포인트만 보강
            AudiogramPoint.objects.bulk_create(
                [AudiogramPoint(session=sess, frequency_hz=int(f)) for f in FREQS_11],
                ignore_conflicts=True,
            )

            visible_freqs = (FREQS_11 if int(sess.points_mode) == 11 else FREQS_7)
            bad = False
            with transaction.atomic():
                for f in visible_freqs:
                    ok_r, r = _parse_db(request.POST.get(f"ag_r_{f}"))
                    ok_l, l = _parse_db(request.POST.get(f"ag_l_{f}"))
                    ok_mr, mr = _parse_db(request.POST.get(f"ag_mr_{f}"))
                    ok_ml, ml = _parse_db(request.POST.get(f"ag_ml_{f}"))
                    ok_br, br = _parse_db(request.POST.get(f"ag_br_{f}"))
                    ok_bl, bl = _parse_db(request.POST.get(f"ag_bl_{f}"))
                    if not ok_r or not ok_l:
                        bad = True
                        continue
                    if not ok_mr or not ok_ml or not ok_br or not ok_bl:
                        bad = True
                        continue
                    AudiogramPoint.objects.update_or_create(
                        session=sess,
                        frequency_hz=int(f),
                        defaults={
                            "right_db": r,
            ...
            if bad:
                _set_required_popup("입력값 오류", ["오디오그램: -10 ~ 120 범위만 입력 가능합니다."])

            # 오류가 없을 때만 edit 상태 해제
            if required_popup is None:
                return redirect(_redirect_url("검사", extra={"ag": sess.id, "scroll": "exam-audiogram"}))

        elif action == "cancel_audiogram":
            if tab != "검사":
                return HttpResponseForbidden("검사 탭에서만 가능합니다.")
            ...
            # 새로 만든 빈 세션이면(ag_is_new=1) 취소 시 자동 삭제
            is_new = (request.POST.get("ag_is_new") == "1")
            if is_new:
                try:
                    pts = list(AudiogramPoint.objects.filter(session=sess))
                    has_any = any(
                        (
                            (p.right_db is not None)
                            or (p.left_db is not None)
                            or (getattr(p, "mask_right_db", None) is not None)
                            or (getattr(p, "mask_left_db", None) is not None)
                            or (getattr(p, "bone_right_db", None) is not None)
                            or (getattr(p, "bone_left_db", None) is not None)
                        )
                        for p in pts
                    )
                except Exception:
                ...
                if not has_any:
                    try:
                        AudiogramPoint.objects.filter(session=sess).delete()
                        sess.delete()
                    except Exception:
                        pass
                    return redirect(_redirect_url("검사", extra={"scroll": "exam-audiogram"}))
        ...
        elif action == "create_audiogram_session":
            if tab != "검사":
                return HttpResponseForbidden("검사 탭에서만 가능합니다.")
            mode_raw = (request.POST.get("ag_points_mode") or "11").strip()
            if mode_raw not in ("7", "11"):
                mode_raw = "11"
            dt_raw = (request.POST.get("ag_test_datetime") or "").strip()
            test_dt = timezone.now()
            if dt_raw:
                try:
                    test_dt = datetime.datetime.fromisoformat(dt_raw)
                except Exception:
                    test_dt = timezone.now()

            sess = AudiogramSession.objects.create(
                customer=customer,
                test_datetime=test_dt,
                points_mode=int(mode_raw),
            )
            # ✅ 항상 11pt 전체 생성(7↔11 전환/히스토리 안정성)
            freqs = FREQS_11
            AudiogramPoint.objects.bulk_create(
                [AudiogramPoint(session=sess, frequency_hz=int(f)) for f in freqs],
                ignore_conflicts=True,
            )
            return redirect(_redirect_url("검사", extra={"ag": sess.id, "ag_edit": "1", "ag_new": "1", "scroll": "exam-audiogram"}))
    ...
    return render(
        ...
        "latest_consultation_id": (latest_consultation.id if latest_consultation else None),
            ...
            # 오디오그램(상담/검사 공통)
            "audiogram_sessions": audiogram_sessions,
            "selected_audiogram": selected_audiogram,
            "audiogram_mode": audiogram_mode,
            "audiogram_freqs": audiogram_freqs,
            "audiogram_points_map": audiogram_points_map,
            "audiogram_rows": audiogram_rows,
            "audiogram_rows_all11": audiogram_rows_all11,
            "exam_ag_has_any": exam_ag_has_any,
            "exam_ag_force_edit": bool(ag_force_edit),
            "exam_ag_is_new": bool(ag_is_new),
        },
    )
